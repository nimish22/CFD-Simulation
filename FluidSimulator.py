...
def simulate(
    ...
    _mosaicSteps = _simulationSteps
    cityRaster = Raster(arcpy.PolygonToRaster_conversion(_boundaryFeatures, _heightField, os.path.join(_outputGDB, "CityRaster"), "MAXIMUM_AREA", _heightField, _cellSize))
    densitySourceRaster = Raster(arcpy.FeatureToRaster_conversion(_densitySourceFeatures, _densityField, os.path.join(_outputGDB, "DensityRaster"), _cellSize))
    uSourceRaster = Raster(arcpy.FeatureToRaster_conversion(_velocitySourceFeatures, _uField, os.path.join(_outputGDB, "URaster"), _cellSize))
    vSourceRaster = Raster(arcpy.FeatureToRaster_conversion(_velocitySourceFeatures, _vField, os.path.join(_outputGDB, "VRaster"), _cellSize))
    ...
    timeStep = 0.1
    cellSize = 1.0 / city.shape[1] #float(cellSize)
    ...
    for i in range(int(_simulationSteps)):
        ...
        if(i%50==0):
            ...
            output.save(os.path.join(_outputGDB, "Frame" + str(i)))
            arcpy.AddMessage("Step Complete")
